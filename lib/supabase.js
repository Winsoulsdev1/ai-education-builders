...
export const WHATSAPP_GROUP_LINK = "https://chat.whatsapp.com/Ej8yjjIBZSALwPRDDE142e?s=cl&p=a&mlu=0";
...
export async function sbSendCode(email) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data.error_description || data.msg || "Couldn't send code.");
  }
}

export async function sbVerifyCode(email, code) {
  ...
  if (!res.ok) throw new Error(data.error_description || data.msg || "Invalid code.");
  return data;
}
